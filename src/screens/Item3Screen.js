...
// ... (Стили остаются без изменений, поэтому я их свернул для краткости)
...
export default function Item3Screen() {
  ...
  // ИЗМЕНЕНО: Используем useState для управления selectedItems, чтобы компонент реагировал на их изменения.
  ...
  const [wishlistViewModalVisible, setWishlistViewModalVisible] =
    useState(false);
  const [categoryDetailsModalVisible, setCategoryDetailsModalVisible] =
    useState(false);
...
const categoryMapRuToEn = {
  ...
  // ДОБАВЛЕНО: Хук для отслеживания изменений в route.params и обновления состояния.
  ...
  //     // Если есть selectedItems, сопоставляем услуги с категориями
  ...
  //             (service) =>
  //               service.serviceType.toLowerCase().replace(/s$/, "") ===
  //               serviceType
  //           );
  ...
  //           );
  //           if (category) {
  //             newCategoryCache[category.id] = matchingServices.map(
  //               (service) => ({
  //                 id: service.id,
  //                 name: service.name,
  ...
  try {
    ...
    // Populate services for selectedItems
    if (selectedItems.length > 0) {
      const newCategoryCache = { ...categoryServicesCache };
      for (const categoryName of selectedItems) {
        const serviceType = categoryMapRuToEn[categoryName];
...
const handleCreateWedding = async () => {
  ...
  //       const weddingsData = Array.isArray(weddingsResponse.data) ? weddingsResponse.data : weddingsResponse.data.data || [];
  ...
  //   };
  //   initialize();
  // }, [token]); // Добавил token в зависимости, т.к. он используется в запросах
  ...
  //     setLoadingWeddings(true);
  //     try {
  //       const [categoriesResponse, weddingsResponse, goodsResponse] =
  //         await Promise.all([
  //           api.getEventCategories(),
  //           api.getWedding(token),
  //           api.getGoods(token),
  ...
  //       // Проверяем selectedItems из route.params
  //       if (route.params?.selectedCategories) {
  //         console.log("Полученные категории:", route.params.selectedCategories);
  //         setSelectedItems(route.params.selectedCategories);

  //         // Создаем категории, если они отсутствуют
  ...
  //       console.log("Fetched Weddings:", weddingsData);
  //       setGoods(
  //         Array.isArray(goodsResponse.data)
  //           ? goodsResponse.data
  ...
  //         );
  //         const servicesResults = await Promise.all(servicesPromises);
  //         const newCategoryCache = servicesResults.reduce(
  //           (acc, { categoryId, services }) => {
  //             acc[categoryId] = services;
  //             return acc;
  //           },
...
useEffect(() => {
  ...
  //                     {(() => {
  //                       const actualService = service.dataValues || service;
  //                       return `${actualService.teamName || actualService.name} ${actualService.cost ? `- ${actualService.cost} тг` : ''}`;
  //                     })()}
  //                   </Text>
  ...
  //                   </View>
  //                 )}
  //                 keyExtractor={(service, index) => `${service.serviceType}-${service.serviceId}-${index}`}
      ...
      <Modal visible={categoryModalVisible} animationType="slide">
        <SafeAreaView style={styles.modalContainer}>
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={styles.noItems}>Услуги недоступны</Text>
            }
        ...
        <SafeAreaView style={styles.modalContainer}>
          {isCustomGift ? (
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <Text style={styles.noItems}>Товары недоступны</Text>
              }
              contentContainerStyle={styles.listContainer}
              initialNumToRender={10}
              windowSize={5}
            ...
            <View style={{ width: "100%" }}>
              ...
              <Text style={styles.detail}>
                ...
                {selectedService.cost
                  ? `${selectedService.cost} тг`
                  : "Не указана"}
              </Text>
              {selectedService.address && (
                <Text style={styles.detail}>
                  Адрес: {selectedService.address}
                </Text>
              )}
              {selectedService.cuisine && (
                <Text style={styles.detail}>
                  Кухня: {selectedService.cuisine}
                </Text>
              )}
              <Text style={styles.detail}>
              ...
              <Text style={styles.detail}>
                ID услуги: {selectedService.serviceId}
              </Text>
            </View>
          ) : (
            <Text style={styles.noItems}>Детали недоступны</Text>
            ...
            <View style={{ width: "100%" }}>
              ...
              <Text style={styles.detail}>
                ...
                {selectedItem.total_cost || selectedItem.cost
                  ? `${selectedItem.total_cost || selectedItem.cost} тг`
                  : "Не указана"}
              </Text>
              {selectedItem.name && (
              ...
              {(selectedItem.created_at || selectedItem.createdAt) && (
                <Text style={styles.detail}>
                  Создано: {selectedItem.created_at || selectedItem.createdAt}
                </Text>
              )}
              {(selectedItem.updated_at || selectedItem.updatedAt) && (
                <Text style={styles.detail}>
                  Обновлено: {selectedItem.updated_at || selectedItem.updatedAt}
                </Text>
              )}
              {selectedItem.serviceId && (
                <Text style={styles.detail}>
                  ID услуги: {selectedItem.serviceId}
                </Text>
              )}
              {selectedItem.wedding_id && (
                <Text style={styles.detail}>
                  ID свадьбы: {selectedItem.wedding_id}
                </Text>
              )}
              {selectedItem.item_id && (
                <Text style={styles.detail}>
                  ID элемента: {selectedItem.item_id}
                </Text>
              )}
              {selectedItem.status && (
