...
export default function Item3Screen() {
  ...
  // Эти строки были пропущены — вот в чём ошибка!
  ...
  const openAddGiftModal = useCallback((type, id) => {
    setGiftTargetType(type);        // 'wedding' или 'eventCategory'
    setGiftTargetId(id);
  ...
  const handleAddWishlistItem = async () => {
    ...
    try {
      const promises = selectedGoodIds.map((goodId) => {
        ...
        console.log('Creating wishlist item with payload:', JSON.stringify(payload, null, 2));
        console.log('giftTargetType:', giftTargetType, 'giftTargetId:', giftTargetId);
        return api.createWish(payload, token);
      });
      ...
      if (giftTargetType === 'wedding') {
          const itemsResponse = await api.getWeddingItems(
            giftTargetId || selectedWedding.id,
            token
          );
          setWeddingItemsCache((prev) => ({
            ...prev,
            [giftTargetId || selectedWedding.id]: itemsResponse.data.data || [],
          }));
      }
    } catch (error) {
